...
pub fn build(b: *std.build.Builder) !void {
    ...
    const toolbox_package_path = "packages/toolbox/src/toolbox.zig";

    const lib = b.addSharedLibrary("pdex", "src/playdate_platform_main.zig", .unversioned);
    lib.addPackagePath("toolbox", toolbox_package_path);
    ...
    game_elf.addPackagePath("toolbox", toolbox_package_path);
    ...
    switch (builtin.target.os.tag) {
        .windows => {
            const copy_assets = b.addSystemCommand(&.{ "cp", "assets/*", "zig-out/Source" });
        ...
        .macos => {
            ...
            const copy_assets = b.addSystemCommand(&.{ "bash", "-c", "cp -r assets/* zig-out/Source/" });
